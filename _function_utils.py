'''
My modified copy of langchain_google_genai/_function_utils.py

Bug Fix: 
1. _convert_pydantic_to_genai_function_declaration to allow use of lists and nested schemas in structured output
'''
...
def _process_properties(schema: Dict[str, Any]) -> Dict[str, Any]:
    parameters = {} 
    type = _get_type_from_schema(schema)
    if type == glm.Type.OBJECT and 'properties' in schema and isinstance(schema['properties'], Dict):
        parameters['properties'] = {}
        for k, v in schema['properties'].items():
            parameters['properties'][k] = _process_properties(v)
        parameters["required"] = schema.get("required", [])
    elif type == glm.Type.ARRAY:
        if not 'items' in schema:
            raise ValueError("You must provide the `items` value for a Schema of type `ARRAY`")
        if isinstance(schema['items'], dict):
            parameters['items'] = _process_properties(schema['items'])
        else:
            parameters['items'] = schema['items']
    parameters['type_'] = type
    parameters['description'] = schema.get('description')
    return parameters
